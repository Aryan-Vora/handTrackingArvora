...
#GOAL
#Find if there is a click and the exact location of the click 
...
def hand_tracking():
    ...
    frame_generated = 0
    # Store hand length
    input_display = np.zeros((720, 1280, 3), dtype=np.uint8)
    
    # List of hand distances for each frame capped at last 120 frames
    hand_distances = [[0, 0, 0]]

    # List of clicks where each click is a tuple of (x, y, frames_left)
    points = 0
    existing = False
    while True:
        ret, img = cap.read()
        ...
        recent_point = hand_distances[-1]

        # Display a graph with the last 120 frames of hand distances
        if len(hand_distances) > 0 and hand_distances[-1][2] != 0:
            # Calculate the size of the dot inversely proportional to the depth
            depth = recent_point[2]
            dot_size = max(1, int(1000 / depth)) 
            cv2.circle(input_display, (int(recent_point[0]), int(recent_point[1])), dot_size, (0, 255, 0), -1)
        ...
        if frame_count % 20 == 0 and not existing:
            target_x = np.random.randint(0, 1280)
            target_y = np.random.randint(0, 720)
        ...
        if existing:
            ...
            if recent_point[0] > target_x and recent_point[0] < target_x + 50 and recent_point[1] > target_y and recent_point[1] < target_y + 50:
