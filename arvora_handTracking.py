...
def hand_tracking():
    ...
    serverAddressPort = ("127.0.0.1", PORT)  # Server IP address and port number
    ...
    # Initialize real_hand_length with default values
    ...
    while True:
        ...
        adjust_z = []
        target_length = 21
        default_value = 0  # Default value to fill, can be None, 0, or other values

        # Initialize adjust_z with default values
            ...
            for handLms in result.multi_hand_landmarks:
                ...
                if cv2.waitKey(1) == ord('t'):
                    ...
                    for index, pair in enumerate(hand_line):
                        i = pair[0]
                        j = pair[1]
                        # process_landmark returns the world coordinates x, y, z, where z refers to tof
                ...
                # Get the depth at the wrist coordinates
                # wristDepth = get_depth_at_coord(x, y, tof_in_hand)
                wristDepth = get_depth_at_coord_wrist(x, y, tof_in_hand)

                # If depth is not available, set adjust_z[0] to 0, otherwise set it to wristDepth
                if wristDepth is None:
                    adjust_z[0] = 0
                else:
                    adjust_z[0] = wristDepth

                # Store the wrist coordinates and depth in the data array
                data[0] = x
                data[1] = y
                data[2] = wristDepth

                # If depth is not available, set wristDepth to 0
                if wristDepth is None:
                    wristDepth = 0

                # Get the next landmark coordinates
                ...
                # Repeat the process for another set of coordinates with a slight depth adjustment
                x = lm.x * imgWidth
                y = lm.y * imgHeight
                z = wristDepth + 2
                world_coor1 = image2World(x, y, z, M, R, t)
                ...
                # Display the second set of coordinates on the image
                text = f"coor2: ({x,y,z})"
                cv2.putText(img, text, (10, 350), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, ), 2)

                # Get the wrist landmark coordinates again
                lm = handLms.landmark[0]
                x = lm.x * imgWidth
                y = lm.y * imgHeight
                z = wristDepth

                # Convert image coordinates to world coordinates for the wrist
                world_coor1 = image2World(x, y, z, M, R, t)
                x = round(world_coor1[0, 0], 3)
                y = round(world_coor1[1, 0], 3)

                # Display the wrist coordinates on the image
                text = f"wrist: ({x,y,z})"
                ...
                if start:
                    for index, pair in enumerate(hand_line):
                        ...
                        # Convert image coordinates to world coordinates for the first landmark
                        ...
                        # Get depth at the second landmark coordinates
                        tof_z = get_depth_at_coord(x1, y1, tof_in_hand)
                        
                        # Convert image coordinates to world coordinates for the second landmark
                        world_coor1 = image2World(x1, y1, z1, M, R, t)
                        world_coor2 = image2World(x1, y1, z1 + 2, M, R, t)
                        
                        # Round the world coordinates for display or further processing
                        ...
                        z2 = wristDepth + 2
                        if tof_z is None:
                            tof_z = 0  # Initialize tof_z to 0 if it is None

                        # Calculate new 3D point based on given parameters and update adjust_z
                    ...
                    # Display new z-coordinate for landmark 5 on the image
                    ...
                    # Display new z-coordinate for landmark 6 on the image
                    ...
                    # Display new z-coordinate for landmark 7 on the image
                    ...
                    # Display new z-coordinate for landmark 8 on the image
                ...
                # Iterate through all landmarks to calculate and display their world coordinates
